...
export function ResetPasswordForm({ onSuccess }: ResetPasswordFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!password) {
      newErrors.password =
        t('password.error.required') || 'La nueva contraseña es obligatoria';
    } else if (!passwordStrength.isValid) {
      newErrors.password =
        t('password.error.requirements') ||
        'La contraseña no cumple los requisitos';
    }

    if (!confirmPassword) {
      newErrors.confirmPassword =
        t('confirmPassword.error.required') || 'La confirmación es obligatoria';
    } else if (password !== confirmPassword) {
      newErrors.confirmPassword =
        t('confirmPassword.error.mismatch') || 'Las contraseñas no coinciden';
    }
    ...
    <Card className='w-full max-w-md border-gradient overflow-hidden'>
      ...
      <CardContent className='p-6'>
        <form onSubmit={handleSubmit} className='space-y-4'>
          ...
          <div className='space-y-2'>
            ...
            {password && (
              <div className='space-y-1'>
                ...
                <div className='space-y-1'>
                  <div
                    className={`flex items-center gap-2 text-xs ${passwordStrength.requirements.minLength ? 'text-green-400' : 'text-slate-500'}`}
                  >
                  ...
                  <div
                    className={`flex items-center gap-2 text-xs ${passwordStrength.requirements.hasUppercase ? 'text-green-400' : 'text-slate-500'}`}
                  >
                  ...
                  <div
                    className={`flex items-center gap-2 text-xs ${passwordStrength.requirements.hasSpecialChar ? 'text-green-400' : 'text-slate-500'}`}
                  >
          ...
          >
            {isLoading
              ? t('loading') || 'Restableciendo...'
              : t('submit') || 'Restablecer contraseña'}
          </Button>
        </form>
      </CardContent>
